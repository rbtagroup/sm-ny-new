drop function if exists public.rb_check_push_rate_limit(text, integer, integer, integer);
drop function if exists private.rb_check_push_rate_limit(text, integer, integer, integer);
...
as $$
declare
  normalized_key text := nullif(trim(p_bucket_key), '');
  safe_weight integer := greatest(1, coalesce(p_weight, 1));
  safe_max integer := greatest(1, coalesce(p_max_count, 30));
  safe_window integer := greatest(1, coalesce(p_window_seconds, 60));
...
begin
  ...
  on conflict on constraint push_rate_limits_pkey do update
  set
    count = case
...
as $$
  select *
  from private.rb_check_push_rate_limit($1, $2, $3, $4);
$$;
