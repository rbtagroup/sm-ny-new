...
function DriverTwoWeekCalendar({ shifts, openShifts, helpers }) {
  ...
  const WeekRow = ({ days, index, interactive = true }) => <div className="driver-week-block">
    ...
    <div className="driver-week-grid">{days.map((day) => {
      ...
      const content = <><b>{weekdayMap[new Date(day).getDay()]?.slice(0,2)}</b><strong>{Number(day.slice(8,10))}</strong><small>{items.some((x) => x.type === 'open') && <Dot type="open" />}{items.some((x) => x.type === 'confirmed') && <Dot type="confirmed" />}{items.some((x) => x.type === 'own') && <Dot type="own" />}</small></>
      return interactive
  ...
  return <div className="card driver-calendar-card">
    ...
    <div className="driver-calendar-legend"><span><Dot type="open" />volná</span><span><Dot type="confirmed" />potvrzená</span><span><Dot type="own" />ostatní moje</span></div>
