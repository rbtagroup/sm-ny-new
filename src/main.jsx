  ...
  const resolveSwap = (id, status) => {
    ...
    if (status === 'approved') {
      ...
      const notices = req.targetMode === 'open'
        ? [makeNotice({ title: 'Byla ti přidělena volná směna', body: `${formatDate(fresh.date)} ${fresh.start}–${fresh.end} · ${helpers.vehicleName(fresh.vehicleId)}`, targetDriverId: newDriverId, type: 'open-shift-approved', shiftId: fresh.id })]
        : [
          ...
          makeNotice({ title: 'Převzal jsi směnu', body: `${formatDate(fresh.date)} ${fresh.start}–${fresh.end} · ${helpers.vehicleName(fresh.vehicleId)}`, targetDriverId: newDriverId, type: 'swap-approved', shiftId: fresh.id }),
        ]
      return commit((prev) => addNotificationsToData({ ...prev, swapRequests: (prev.swapRequests || []).map((r) => r.id === id ? appendSwapHistory({ ...r, status, resolvedAt: new Date().toISOString(), approvedDriverId: newDriverId }, `Admin schválil převzetí pro ${helpers.driverName(newDriverId)}.`) : r), shifts: prev.shifts.map((s) => s.id === fresh.id ? { ...s, driverId: newDriverId, status: 'assigned', swapRequestStatus: 'approved' } : s) }, notices), `${req.targetMode === 'open' ? 'Volná směna byla přidělena' : 'Výměna schválena a směna převedena na'} ${helpers.driverName(newDriverId)}.`)
    }
