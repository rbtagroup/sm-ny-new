...
const states = [
  ...
  ['staff', 'drivers-remove', [staffNav('Řidiči'), clickFirst('.list-row-main'), click('.shift-drawer button', 'Trvale odstranit řidiče')]],
  ['staff', 'drivers-deactivate', [staffNav('Řidiči'), clickFirst('.list-row-main'), click('.shift-drawer button', 'Vyřadit řidiče')]],
  ...
  ['staff', 'availability-new', [staffNav('Řidiči'), click('button', 'Dostupnost a nepřítomnost'), clickFirst('.availability-add')]],
  ['staff', 'notifications-compose', [staffNav('Notifikace'), click('button', 'Nová zpráva řidičům')]],
  ['staff', 'settings-diagnostics', [staffNav('Nastavení'), `(() => { const details = document.querySelector('.settings-diagnostics'); if (details) details.open = true; return Boolean(details) })()`]],
  ['staff', 'more-sheet', [staffNav('Plán směn'), `(() => { const button = [...document.querySelectorAll('.staff-bottom-nav button')].find((item) => item.innerText.includes('Více')); button?.click(); return Boolean(button) })()`]],
